...
;	.pisend -c [cmd]
;		Issues a bash command to NextPi 
;	.pisend -r 
...
secondbit:	        cp 'q'
                        ...
                        cp 'S'
                        jp z,quietmodetrue			; -S no output
...
.exitcopy			
                        ...
                        ld a,1 : ld (quietmode),a :	jp uploadmode 		
...
.toutb:		; if busy do a little loop 
                        ...
                        jr      z, 1F 
                        in      a,(c)                        
                        ; locks up with no pi0 on S6
                        bit     4, a 
                        ; if z =  no, bit 4 TX BUFFER NOT EMPTY 
                        ; if nz = yes, bit 4 TX BUFFER EMPTY 
...
print_rst16	;ld a,(quietmode) : bit 0,a : ret nz 

.subprint
                        ld a,(hl):inc hl:or a:ret z:rst 16:
...
textbuff    	ds 		32,0
version			db		22,21,0,16,3,VERSION,22,0,0,255
emptyline		db		".pisend ",VERSION," - David Saphier",13,13,"usage :",13,13,"pisend [-q][-c][-s][-S][-r][file]",13,13
...
linetwo			db 		"use:",13,13
                                ...
                                db 		".pisend -s[char]",13,13
                                db		" sends a char to the terminal",13
                                db 		13,13 
                                
                                db 		".pisend -u",13,13
                                db		" attempts to swap permanent speed",13
                                db		" between 115200 and 2000000",13
                                db		" change this as your own risk!",13
                                db 		13,13 
...
txtchunks		db		22,5,1,"chunks : ",13,22,5,11,0
...
update2mb		db "nextpi-admin_enable",$0d,$0a
                                ...
                                db "echo dwc_otg.lpm_enable=0 console=serial0,1152000 console=tty1 root=PARTUUID=b8ef7a27-02 "
...
end115
_2mbto115:		db 13,"Swapping 115,2000 Mbit to 115,200",0
_115to2mb: 		db 13,"Swapping 115,200 to 115,2000",0
...
.endmarker
        ...
        savebin "pisend",$2000,.endmarker-$2000
        savebin "h:/dot/pisend",$2000,.endmarker-$2000
